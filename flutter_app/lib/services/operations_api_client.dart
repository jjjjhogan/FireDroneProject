  ...
  factory MapProviderConfig.fromJson(Map<String, dynamic> json) {
    ...
    return MapProviderConfig(
      ...
      basemaps: (json['basemaps'] as List<dynamic>? ?? const [])
          ...
          .toList(),
      centerLat: (center['lat'] as num?)?.toDouble() ?? 37.211,
      centerLng: (center['lng'] as num?)?.toDouble() ?? -119.54,
    ...
    return const GeofenceLayer(
      ...
      features: [
        GeofenceFeature(
          ...
          coordinates: [
            [
              MapCoordinate(lat: 37.2050, lng: -119.5560),
              MapCoordinate(lat: 37.2210, lng: -119.5480),
              MapCoordinate(lat: 37.2180, lng: -119.5280),
              MapCoordinate(lat: 37.2080, lng: -119.5220),
              MapCoordinate(lat: 37.2020, lng: -119.5300),
              MapCoordinate(lat: 37.2030, lng: -119.5480),
              MapCoordinate(lat: 37.2050, lng: -119.5560),
            ],
          ],
        ),
        GeofenceFeature(
          ...
          coordinates: [
            [
              MapCoordinate(lat: 37.1990, lng: -119.5640),
              MapCoordinate(lat: 37.1985, lng: -119.5160),
              MapCoordinate(lat: 37.2245, lng: -119.5140),
              MapCoordinate(lat: 37.2250, lng: -119.5620),
              MapCoordinate(lat: 37.1990, lng: -119.5640),
            ],
          ],
        ),
  ...
  factory MapMissionLayer.unavailable() {
    return const MapMissionLayer(
      ...
      route: MapMissionRoute(
        id: 'local-min-mountains-route',
        name: 'Local Min Mountains patrol',
        source: 'local-fallback',
        points: [
          MapRoutePoint(
            label: 'LZ Alpha',
...
class MapBounds {
  ...
  factory MapBounds.fromJson(Object? json) {
    ...
    return MapBounds(
      ...
      north: (data['north'] as num?)?.toDouble() ?? 37.211,
      south: (data['south'] as num?)?.toDouble() ?? 37.211,
      east: (data['east'] as num?)?.toDouble() ?? -119.54,
      west: (data['west'] as num?)?.toDouble() ?? -119.54,
    );
  }
